...
export async function POST(req: Request) {
  ...
  if (!user) return Response.json({ error: "unauthorized", message: "Connectez-vous." }, { status: 401 });
  ...
  try {
    ...
    if (!result.accepted) {
      return Response.json(
        { error: "provider_rejected", message: "PawaPay a refusé l'initiation." },
        { status: 502 },
      );
    }
    ...
    return Response.json(
      {
        ...
        message:
          msg === "pawapay_not_configured"
            ? "PawaPay n'est pas encore configuré sur ce serveur."
